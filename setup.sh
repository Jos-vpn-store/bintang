...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To jos-vpn-store Project Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}jos-vpn-store ${NC}${YELLOW}(${NC} ${green}jos-vpn-store Project ${NC}${YELLOW})${NC}"
echo -e "       Â© Recode By jos-vpn-store Project ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/jos-vpn-store/bulanbintang/main/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/jos-vpn-store"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/jos-vpn-store/bulanbintang/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/jos-vpn-store/bulanbintang/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/jos-vpn-store/bulanbintang/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/jos-vpn-store/bintang/main/"
...
function password_default() {
    ...
    ============================
    (C) Copyright 2023 By jos-vpn-store Project
    ============================
"
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/jos-vpn-store/limit/vmess/ip
    mkdir -p /etc/jos-vpn-store/limit/vless/ip
    mkdir -p /etc/jos-vpn-store/limit/trojan/ip
    mkdir -p /etc/jos-vpn-store/limit/ssh/ip
